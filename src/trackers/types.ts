...
export interface IssueTracker {
   ...
   * tool so the agent can signal completion without shelling out to `mv`. Trackers that
   * are read-only (e.g. external services accessed via a token without write scope) may
   * omit this; the MCP server will reject `mark_done` calls in that case.
   *
   ...
   * cannot redirect a `mark_done` to a different filesystem location.
   *
