...
// pure executor consumes the port and the runner wires `realRunInVmCacheStore`.
//
// Cache layout:
...
import type { RunInVmCacheKey, RunInVmCachedResult, RunInVmCacheStore } from './types.js';
 ...
 */
// eslint-disable-next-line max-statements -- workspace hash inlined for clarity; splitting helped no reader.
...
/** CLI shape: drop every cache entry for a named `run_in_vm` action. */
...
/** Bind the cache adapter to a specific on-disk cacheRoot. */
...
/** Production cache store, wired by the runner into runActions. */
