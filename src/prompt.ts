// Strict Liquid rendering for the per-issue prompt template (SPEC §8, §4.4).
// Unknown variables and unknown filters must fail rendering (§4.4).
...
function issueToScope(issue: Issue): Record<string, unknown> {
  // Convert keys to strings for template compatibility (§8.2). Preserve nested arrays/maps.
  return {
    id: issue.id,
...
export async function renderPrompt(input: RenderInput): Promise<string> {
  ...
  if (tpl.length === 0) {
    // §4.4 fallback prompt when workflow body is empty.
