...
const domain      = 'src/reconciler/(vm|workspace|pr)\.ts$|src/actions/(executor|predicates|templating|index)\.ts$|src/(workspace|mcp|issues|prompt)\.ts$';
const adapters    = 'src/trackers/local\.ts$|src/agent/(smolvm|adapters|acp|tool-call-summary)\.ts$|src/(acp-bridge|runlog|memory)\.ts$|src/reconciler/bake\.ts$|src/actions/cache\.ts$';
