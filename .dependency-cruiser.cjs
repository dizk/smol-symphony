...
const adapters    = 'src/trackers/local\.ts$|src/agent/(smolvm|gondolin|adapters|acp|tool-call-summary|credential-proxy|credential-secrets|credential-ticker)\.ts$|src/(acp-bridge|runlog|memory|workspace)\.ts$|src/reconciler/bake\.ts$|src/actions/predicate-env\.ts$';
