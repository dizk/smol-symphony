...
const foundation  = 'src/(types|logging|errors|workspace-types)\.ts$|src/util/[^/]+\.ts$|src/(actions|reconciler|trackers)/types\.ts$|src/reconciler/(cache|bake-plan|pr-decide)\.ts$|src/actions/cache\.ts$';
const config      = 'src/workflow\.ts$|src/actions/parsing\.ts$';
const domain      = 'src/reconciler/(vm|workspace|pr)\.ts$|src/actions/(executor|predicates|templating|index)\.ts$|src/(mcp|issues|prompt)\.ts$';
const adapters    = 'src/trackers/local\.ts$|src/agent/(smolvm|gondolin|gondolin-creds-staging|adapters|acp|tool-call-summary|credential-proxy|credential-secrets|credential-ticker)\.ts$|src/(acp-bridge|runlog|memory|workspace)\.ts$|src/reconciler/bake\.ts$|src/actions/predicate-env\.ts$';
const application = 'src/orchestrator\\.ts$|src/reconciler/index\\.ts$|src/agent/(runner|gondolin-dispatch)\\.ts$';
