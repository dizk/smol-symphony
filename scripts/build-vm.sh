...
#
# Plus the symphony-vm-agent stdio proxy at /opt/symphony/vm-agent.mjs (see vm-agent.mjs).
#
...
smolvm machine exec --name "$VM_NAME" -i -- bash -lc 'mkdir -p /opt/symphony && cat > /opt/symphony/vm-agent.mjs && chmod 0644 /opt/symphony/vm-agent.mjs' < "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/vm-agent.mjs"
